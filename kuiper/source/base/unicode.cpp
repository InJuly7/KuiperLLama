...
uint32_t unicode_cpt_from_utf8(const std::string &utf8, size_t &offset) {
    ...
    if (!(utf8[offset + 0] & 0x08)) {
        ...
        auto result =
            ((utf8[offset + 0] & 0x07) << 18) | ((utf8[offset + 1] & 0x3f) << 12) | ((utf8[offset + 2] & 0x3f) << 6) | (utf8[offset + 3] & 0x3f);
        offset += 4;
        return result;
    }
...
static std::vector<size_t> unicode_regex_split_custom_gpt2(const std::string &text, const std::vector<size_t> &offsets) {
    ...
    for (auto offset : offsets) {
        ...
        auto _get_cpt = [&](const size_t pos) -> uint32_t { return (offset_ini <= pos && pos < offset_end) ? cpts[pos] : OUT_OF_RANGE; };
...
std::vector<std::string> unicode_regex_split(const std::string &text, const std::vector<std::string> &regex_exprs) {
    ...
    // generate a "collapsed" representation of the text, where all codepoints
    // are replaced by a single byte ref:
    ...
    if (need_collapse) {
        ...
        for (size_t i = 0; i < cpts.size(); ++i) {
            ...
            if (flags.is_whitespace) {
                // NOTE: C++ std::regex \s does not mach 0x85, Rust and Python
                // regex does. text_collapsed[i] = (char) 0x85;  // <Next Line>
                // as whitespace fallback
    ...
    for (auto &regex_expr : regex_exprs) {
        ...
        try {
            // if a unicode category is used in the regex, we use the collapsed
            // text and replace the unicode category with the corresponding
            ...
            if (use_collapsed) {
                // sanity-check that the original regex does not contain any
                // non-ASCII characters
                const auto cpts_regex = unicode_cpts_from_utf8(regex_expr);
                for (size_t i = 0; i < cpts_regex.size(); ++i) {
                    if (cpts_regex[i] >= 128) {
                        throw std::runtime_error(
                            "Regex includes both unicode categories and "
                ...
                // std::wregex \s does not mach non-ASCII whitespaces, using
                // 0x0B as fallback
