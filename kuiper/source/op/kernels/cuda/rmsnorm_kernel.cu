...
static __global__ void row_rmsnorm_f32(float *in, float *wei, float *out, int size, float eps) {
    ...
    for (int i = tid; i < pack_num; i += blockDim.x) {
        ...
        *(out_pack + i) = make_float4(scale * in_float4.x * wei_float4.x, scale * in_float4.y * wei_float4.y, scale * in_float4.z * wei_float4.z,
...
void rmsnorm_kernel_cu(const tensor::Tensor &input, const tensor::Tensor &weight, const tensor::Tensor &output, void *stream) {
    ...
    CHECK(input.device_type() == base::DeviceType::kDeviceCUDA && weight.device_type() == base::DeviceType::kDeviceCUDA &&
