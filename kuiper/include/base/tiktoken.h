...
static auto _byte_pair_merge(const std::string &piece, const ankerl::unordered_dense::map<std::string, int> &ranks, std::function<int(int, int)> func)
    -> std::vector<int> {
...
class tiktoken {
   public:
    ...
    tiktoken(ankerl::unordered_dense::map<std::string, int> encoder, ankerl::unordered_dense::map<std::string, int> special_encoder,
        ...
        // assert(encoder_.size() != decoder_.size() && "Encoder and decoder
        // must be of equal length; maybe you had duplicate token indices in
        // your encoder?");
   ...
   private:
    ...
    auto _encode_native(const std::string &text, const ankerl::unordered_dense::map<std::string, int> &allowed_special) const
