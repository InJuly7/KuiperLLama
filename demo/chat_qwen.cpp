...
class ChatAssistant {
   public:
    ChatAssistant(const std::string &model_path, const std::string &tokenizer_path) : model_path_(model_path), tokenizer_path_(tokenizer_path) {}

    // 初始化模型
    bool init() {
        try {
            model_ = std::make_unique<model::Qwen2Model>(base::TokenizerType::kEncodeBpe, tokenizer_path_, model_path_, false);
        ...
        while (pos < max_length) {
            ...
            if (is_prompt) {
                ...
                if (pos >= 3) {
                    ...
                    if (decoded.find("<|im_end|>") != std::string::npos || decoded.find("<|endoftext|>") != std::string::npos) {
                        break;
                    }
                }
            }
